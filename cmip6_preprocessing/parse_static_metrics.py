...
def extract_all_availabe_metrics(
    col, grid_label, source_id, varname="areacello", **kwargs
):
...
def extract_static_metric(
    ...
    # Find an area for as many models as possible (no matter in which scenario, ensemble etc)
    subset = col.search(
        variable_id=[varname], grid_label=grid_label, source_id=source_id
    )
    ...
    if len(subset.df) < 1:
        ...
        if len(metric_dict) == 0:
            warnings.warn(
                "No metric [%s] found for source_id [%s] and grid_label[%s]"
                % (varname, source_id, grid_label)
            )
            return None
        elif len(metric_dict) > 1:
            print(metric_dict)
            raise RuntimeError(
                "Something went reaalllly wrong. Metric dict should only have one key"
            )
        else:
...
def parse_metrics(data_dict, col, varname="areacello", preprocess=None, rename=False):
    ...
    for k, ds in data_dict.items():
        metric = extract_static_metric(
            col, ds.attrs["grid_label"], ds.attrs["source_id"], preprocess=preprocess
        )
